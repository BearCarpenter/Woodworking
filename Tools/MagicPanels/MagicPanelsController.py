...
def panelFace(iType):
  try:
    ...
    panel.Placement = FreeCAD.Placement(
      FreeCAD.Vector(x, y, z), FreeCAD.Rotation(0, 0, 0)
    )
...
def panelBetween(iType):
  try:
    ...
    [panel.Length, panel.Width, panel.Height] = MagicPanels.sizesToCubePanel(
      obj1Ref, iType
    )
    ...
    panel.Placement = FreeCAD.Placement(
      FreeCAD.Vector(x1, y1, z1), FreeCAD.Rotation(0, 0, 0)
    )
...
def panelBackOut():
  try:
    ...
    if L > 0 and W > 0 and H > 0:
      ...
      panel.Placement = FreeCAD.Placement(
        FreeCAD.Vector(x1, y1, z3), FreeCAD.Rotation(0, 0, 0)
      )
  ...
  try:
    ...
    for o in selectedObjects:
      ...
      if o.isDerivedFrom("Part::Box"):
        ...
        for s in selectedSubs[o]:
          if s.ShapeType == "Edge":
            [edge, edgeName, edgeIndex] = MagicPanels.getSubByKey(
              pad, [s.BoundBox], "BoundBox", "edge"
            )
            subs.append(edge)

          if s.ShapeType == "Face":
            [face, faceName, faceIndex] = MagicPanels.getSubByKey(
              pad, [s.BoundBox], "BoundBox", "face"
            )
            subs.append(face)
